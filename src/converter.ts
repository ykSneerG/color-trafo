...
import { SpectralData, Cmf, CIE1931_2DEG } from './constants/cmfs.js';
 ...
 * @internal
 */
function interpolateSpectral(spectralData: SpectralData, targetWavelengths: number[]): number[] {
    const { wavelengths: srcWL, values: srcValues } = spectralData;
    
    return targetWavelengths.map(targetWL => {
        // Find surrounding points
        let i = 0;
        while (i < srcWL.length - 1 && srcWL[i + 1] < targetWL) i++;
        
        // Exact match
        if (srcWL[i] === targetWL) return srcValues[i];
        
        // Outside range - return 0
        if (targetWL < srcWL[0] || targetWL > srcWL[srcWL.length - 1]) return 0;
